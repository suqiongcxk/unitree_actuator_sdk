...
int main()
{
    ...
    if (!clamped_hold.hold_clamped || !clamped_hold.clamped
            ...
            || clamped_hold.effective_hold_ns != 2000 * ms) return 1;
    manager.update(2999 * ms);
    if (manager.status(2999 * ms).timed_out) return 1;
    manager.update(3001 * ms);
    if (!manager.status(3001 * ms).timed_out) return 1;
