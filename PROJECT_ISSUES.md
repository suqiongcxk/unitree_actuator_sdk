...
| ISSUE-002 | P0 | NN 安全验证 | `JOINT_LIMITS` 使用逐腿排列，但验证代码按数组下标/motor ID 访问 | 待确认 |
...
- 当前进展（2026-08-09）：已加入进程级急停标志、键盘/信号触发、总线阻尼锁存、12 电机周期阻尼和幂等安全退出；过温、反馈陈旧、deadline miss 等自动保护仍待实现。
