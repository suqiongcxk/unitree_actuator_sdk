...
| ISSUE-004 | P0 | 并发通信 | `DoubleBuffer` 无法严格保证读指针使用期间对应槽不被覆盖 | 待确认 |
...
| ISSUE-010 | P0 | 电机力矩 | GO-M8010-6 已是关节输出扭矩，却被重复应用减速比 | 已解决 |
...
- 当前进展（2026-08-13）：已加入进程级急停、总线阻尼锁存、反馈 CRC/ID 检查、每电机反馈时间戳、100 ms 新鲜度阈值、连续失败计数和 NN 状态流看门狗；过温阈值与控制线程 deadline miss 统计仍待实现。

## ISSUE-010：GO-M8010-6 力矩被重复应用减速比

- 优先级：P0
- 位置：`motor_controller.cpp`、`parallel_bus.cpp` 的命令和反馈换算。
- 原因：位置/速度按 SDK 示例需要进行转子端与输出端换算，但 GO-M8010-6 协议结构明确把 `tor_des` 和 `torque` 定义为关节输出扭矩 N·m。
- 风险：反馈力矩被错误放大一个减速比，力矩指令又被错误缩小，影响接触检测、安全阈值和后续状态估计。
- 处理结果（2026-08-13）：保留位置/速度减速比换算，取消力矩的额外乘除；单电机、同步总线和并行总线保持一致。
- 测试结果：所有相关目标编译通过，URDF 力矩方向往返测试通过；真实力矩精度仍需用已知载荷标定。
